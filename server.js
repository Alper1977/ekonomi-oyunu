...
setInterval(() => {
    try {
        ...
        // -------------------------------------------------------------
        // 1. BOT VE GLOBAL İLAN/PİYASA İŞLEMLERİ (Sunucu Tarafı)
        // -------------------------------------------------------------
        try {
...
for (let i = 0; i < 5; i++) {
    ...
    if (Math.random() < 0.50) { // Şansı biraz artır ki hemen aksiyon alılsın
        ...
        if (rastgeleBot.nakit >= bedel) {
            ...
            // --- C. Diğer Botların İlanlarının Süresi Dolması ---
            ...
            const saveBotTransaction = db.transaction((botListesi) => {
                botListesi.forEach(b => {
                    updateStmt.run(b.nakit, JSON.stringify(b.varliklar), b.id);
                });
            });
            saveBotTransaction(botlar);

        } catch (botErr) {
            console.error("Bot ve piyasa döngüsü hatası:", botErr.message);
        }

        // -------------------------------------------------------------
        // 2. KULLANICI EKONOMİSİ VE İLAN SATIN ALMA İŞLEMLERİ
        // -------------------------------------------------------------
        const kullanicilar = db.prepare(`SELECT id, portfoy, krediler, son_guncelleme FROM kullanicilar`).all();
        ...
        kullanicilar.forEach(user => {
            try {
                const userTransaction = db.transaction(() => {
                    // Önce kullanıcının kendi aktif ilanlarının botlar tarafından garanti satış kontrolü yapılabilir
                    // Veya mevcut ekonomi fonksiyonu çağrılır:
...
app.post('/api/ilan-sil', (req, res) => {
    ...
    try {
        const transaction = db.transaction(() => {
            // 1. Kullanıcının portföyünden mülkü bulup durumunu 'sahip' yapalım 
            // ve varsa veritabanındaki eşleşen ilan ID'sini yakalayalım
            const userRow = db.prepare(`SELECT portfoy FROM kullanicilar WHERE id = ?`).get(userId);
            let silinecekIlanIds = [];
            if (id) silinecekIlanIds.push(id);
            if (sunucuIlanId) silinecekIlanIds.push(sunucuIlanId);
            ...
            if (userRow && userRow.portfoy) {
                ...
                if (portfoyObj && portfoyObj.varliklar && Array.isArray(portfoyObj.varliklar)) {
                    portfoyObj.varliklar.forEach(v => {
                        // Varlık ID'si veya sunucuIlanId eşleşiyorsa
                        if (
                            (id && (v.id == id || v.sunucuIlanId == id)) || 
                            (sunucuIlanId && (v.id == sunucuIlanId || v.sunucuIlanId == sunucuIlanId))
                        ) {
                            if (v.sunucuIlanId) {
                                silinecekIlanIds.push(v.sunucuIlanId);
                            }
                            v.durum = 'sahip';
                            v.ilanSahibi = null;
                            v.sunucuIlanId = null;
            ...
            // 2. Toplanan tüm olası ilan ID'lerine göre ilanlar tablosunu tertemiz sil
            if (silinecekIlanIds.length > 0) {
                const placeholders = silinecekIlanIds.map(() => '?').join(',');
                db.prepare(`DELETE FROM ilanlar WHERE kullanici_id = ? AND id IN (${placeholders})`).run(userId, ...silinecekIlanIds);
            }

            // Ek güvenlik: Eğer ilan_tipi gönderildiyse isme göre de kalıntı bırakma
            if (ilan_tipi) {
                db.prepare(`DELETE FROM ilanlar WHERE kullanici_id = ? AND (ilan_tipi = ? OR baslik LIKE ?)`).run(userId, ilan_tipi, `%${ilan_tipi}%`);
            }
        });
...
app.get('/api/zenginler-listesi-ortak', (req, res) => {
    try {
        ...
        } catch (e) { /* Tablo yoksa varsayılanla devam et */ }

        // 3. Gerçek kullanıcıları güvenle çek ve servetlerini hesapla
        let kullaniciListesi = [];
        try {
            const kullanicilarRows = db.prepare(`SELECT id, kadi, adsoyad, portfoy FROM kullanicilar`).all();
            kullaniciListesi = kullanicilarRows.map(u => {
                let portfoy = {};
                try {
                    if (u.portfoy) {
                        portfoy = typeof u.portfoy === 'string' ? JSON.parse(u.portfoy) : u.portfoy;
                    }
                } catch (err) { portfoy = {}; }
        ...
        try {
            ...
            botListesi = botlarRows.map(b => {
                ...
                let portfoy = {
                    nakit: Number(b.nakit) || 0,
                    vadeli: Number(b.vadeli) || 0,
                    altin: Number(b.altin) || 0,
                    dolar: Number(b.dolar) || 0,
                    euro: Number(b.euro) || 0,
                    kredi: Number(b.kredi) || 0,
                    varliklar: Array.isArray(varliklarDizisi) ? varliklarDizisi : []
                };
                ...
                return {
                    id: b.id,
                    isim: b.isim || "İsimsiz Bot",
                    servet: Number(toplamServet) || 0,
                    tip: "Bot"
                };
            });
        } catch (err) {
            console.error("Botlar listelenirken hata:", err.message);
        }

        // 5. Hepsini birleştir, büyükten küçüğe sırala ve gönder
        ...
        console.error("Zenginler listesi genel kritik hata:", err.message);
        res.status(500).json([]); // Hata anında boş dizi dönerek arayüzün patlamasını önle
    }
});
