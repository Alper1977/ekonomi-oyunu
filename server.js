...
// İlanlar tablosunu oluşturan kod (Veritabanı bağlantısından hemen sonraya ekle)
db.prepare(`CREATE TABLE IF NOT EXISTS ilanlar_havuzu (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    satici_id INTEGER,
    satici_adsoyad TEXT,
    mulk_adi TEXT,
...
app.post('/api/ilan-ver', (req, res) => {
    ...
    const { mulkAdi, fiyat, detaylar } = req.body;
    const saticiId = req.session.kullanici.id;
    const saticiAdSoyad = req.session.kullanici.adsoyad;

    try {
        db.prepare(`INSERT INTO ilanlar_havuzu (satici_id, satici_adsoyad, mulk_adi, fiyat, detaylar) VALUES (?, ?, ?, ?, ?)`)
          .run(saticiId, saticiAdSoyad, mulkAdi, fiyat, detaylar || '');
        
        res.json({ basari: true, mesaj: "İlan başarıyla pazara eklendi." });
    } catch (err) {
        res.status(500).json({ basari: false, mesaj: err.message });
    }
});

// 2. İlanları Listeleme Rotası (Herkes görebilir)
app.get('/api/ilanlari-getir', (req, res) => {
    try {
        const ilanlar = db.prepare(`SELECT * FROM ilanlar_havuzu ORDER BY id DESC`).all();
...
app.post('/api/ilan-satin-al', (req, res) => {
    ...
    // Alıcı ve satıcı verilerini veritabanından güncel çekelim
    const aliciRow = db.prepare(`SELECT * FROM kullanicilar WHERE id = ?`).get(aliciId);
    const saticiRow = db.prepare(`SELECT * FROM kullanicilar WHERE id = ?`).get(ilan.satici_id);

    let aliciPortfoy = JSON.parse(aliciRow.portfoy || '{}');
    let saticiPortfoy = JSON.parse(saticiRow.portfoy || '{}');

    aliciPortfoy.para = aliciPortfoy.para || 0;

    if (aliciPortfoy.para < ilan.fiyat) {
        return res.status(400).json({ basari: false, mesaj: "Yeterli bakiyeniz yok!" });
    }

    // Para transferi ve mülk transferi
    aliciPortfoy.para -= ilan.fiyat;
    saticiPortfoy.para = (saticiPortfoy.para || 0) + ilan.fiyat;

    // Mülkü alıcının varlıklarına ekle
    aliciPortfoy.varliklar = aliciPortfoy.varliklar || [];
    ...
    try {
        // Veritabanı işlemlerini transaction ile güvenli yapalım
        const transaction = db.transaction(() => {
            // Alıcıyı güncelle
            db.prepare(`UPDATE kullanicilar SET portfoy = ? WHERE id = ?`).run(JSON.stringify(aliciPortfoy), aliciId);
            // Satıcıyı güncelle
            db.prepare(`UPDATE kullanicilar SET portfoy = ? WHERE id = ?`).run(JSON.stringify(saticiPortfoy), ilan.satici_id);
            // İlanı havuzdan sil
            db.prepare(`DELETE FROM ilanlar_havuzu WHERE id = ?`).run(ilanId);
        });
        ...
        res.json({ basari: true, mesaj: "İlan başarıyla satın alındı ve mülk portföyünüze eklendi!" });
    } catch (err) {
        res.status(500).json({ basari: false, mesaj: "İşlem sırasında hata oluştu: " + err.message });
    }
});
